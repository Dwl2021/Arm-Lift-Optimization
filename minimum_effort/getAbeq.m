function [Aeq, beq]= getAbeq(n_seg, n_order, ts, start_cond, end_cond)
    ...
    Aeq_start(1 : state_num, 1 : n_order+1) = getCoeff(0, n_order); 
    ...
    t = ts(end);
    getCoeff(t, n_order)
    Aeq_end(1 : state_num, n_all_poly - n_order : n_all_poly) = getCoeff(t, n_order);
    ...
    for k = 0:1:n_seg-3 % here k is the index of segments
            Aeq_con(1+n*k:n+n*k,1+2*n*k:2*n+2*n*k) = getCoeff(ts(k+1), n_order);
            Aeq_con(1+n*k:n+n*k,1+2*n*(k+1):2*n+2*n*(k+1)) = -getCoeff(0, n_order);            
    end
